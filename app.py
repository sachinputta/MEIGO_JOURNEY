...
def registeruser():
    ...
    if x==None:
        ...
        db.commit()
        return(jsonify({ 'userid':registeremail,'message': 'Registered successfully'}))
    else:
        return(jsonify({'userid':registeremail, 'message': 'user already exists'}))
...
def getjourneydetails():
    ...
    if x:
        ...
        # print(y)
        if y:
            cur.execute("SELECT journeyID,journeyDATE,Emotion_tag,userupload, Activity,Others  from  MG_JRNY_Details  where   journeyID=%s ",(journeyid,))
            z=cur.fetchall()
            print(z)
            return(jsonify({'dailytasks': z}))
